  ...
  config = function()
    ...
    local snippets = {
      ...
      spi = p({
        trig = "spi",
        name = "print(|)",
        dscr = "surrounds with print function informative",
      }, 'print("$TM_SELECTED_TEXT: ", $TM_SELECTED_TEXT$1)'),
      sc = p({
        trig = "sc",
        name = "surronds with {|}",
    ...
    -- python surround with print()
    map_snippet(";ci", mappable.spi)
    vim.api.nvim_set_keymap("v", ";ci", "<c-s>;pi", { noremap = false, silent = false })
  end,
}
